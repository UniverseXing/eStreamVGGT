#!/usr/bin/env python3
"""Validate and summarize the frozen Stage 5A experiment matrix."""
...
def load_video_depth(root: Path, dataset: str, method: str, allow_incomplete: bool):
    pattern = str(root / "eval_results/video_depth" / f"{dataset}_streamvggt*stage5a_{method}" / "runtime_memory_rank0.json")
...
def load_pose(root: Path, method: str, allow_incomplete: bool):
    pattern = str(root / "eval_results/pose" / f"tum_streamvggt_*stage5a_{method}" / "pose_metrics.json")
...
def load_reconstruction(root: Path, method: str, allow_incomplete: bool):
    pattern = str(root / "eval_results/mv_recon" / f"streamvggt_stage5a_{method}" / "reconstruction_metrics.json")
...
def main():
    ...
    if len(gpu_names) > 1:
        raise RuntimeError(f"Stage 5A mixes GPU models: {sorted(gpu_names)}")
    if not args.allow_incomplete:
        if len(gpu_names) != 1 or "6000 ada" not in next(iter(gpu_names)).lower():
            raise RuntimeError(f"formal Stage 5A requires one RTX 6000 Ada GPU model: {gpu_names}")
    ...
    write_csv(root / "stage5a_same_budget_results.csv", COMMON_FIELDS, sorted(same_aggregate, key=aggregate_key))
    write_csv(root / "stage5a_same_budget_sequence_results.csv", SEQUENCE_FIELDS, sorted(same_sequence, key=sequence_key))
    write_csv(root / "stage5a_component_results.csv", COMMON_FIELDS, sorted(component_aggregate, key=aggregate_key))
    write_csv(root / "stage5a_paired_statistics.csv", PAIRED_FIELDS, paired_rows(sequence_rows, args.bootstrap_samples))
