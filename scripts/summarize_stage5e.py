#!/usr/bin/env python3
"""Summarize the emergency K4 versus official StreamVGGT-STAC comparison."""
...
def load_k4(root: Path, datasets):
    ...
    if "sq_rel" not in aggregate_sources[0]:
        aggregate_path = root / "supplementary material/tables/p0_04_depth_secondary_metrics_summary.csv"
        sequence_path = root / "supplementary material/tables/p0_04_depth_secondary_metrics_sequences.csv"
        aggregate_sources = read_csv(aggregate_path)
        sequence_sources = read_csv(sequence_path)
    aggregates = []
    for source in aggregate_sources:
        method_id = source.get("method_id", source.get("method"))
        if source["dataset"] not in datasets or method_id not in ("k4", "proposed_k4"):
            continue
        ...
        row.update(
            ...
            peak_reserved_mb=source.get("peak_reserved_mb", source.get("peak_reserved_mib")),
...
def load_stac(root: Path, output_root: Path, datasets):
    ...
    for dataset in datasets:
        ...
        row = blank(RESULT_FIELDS)
        row.update(
            dataset=dataset, method="StreamVGGT-STAC",
            ...
            cuda_version=summary["cuda_version"], stac_commit=summary["stac_commit"],
        ...
        for item in sequence_metrics["sequences"]:
            ...
            sequence_row.update(
                dataset=dataset, sequence=sequence, method="StreamVGGT-STAC",
...
def parity_gate(root: Path, output_root: Path):
    ...
    stac_path = output_root / "parity_stac/bonn/result_scale.json"
    if len(ours_candidates) != 1 or not stac_path.is_file():
        return {
    ...
    stac = float(read_json(stac_path)["Abs Rel"])
    tolerance = max(abs(ours) * 0.02, 0.002)
    difference = abs(ours - stac)
    return {
        "check": "streamvggt_full_parity", "passed": int(difference <= tolerance),
        "observed": f"ours={ours:.8g}; stac_adapter={stac:.8g}; abs_diff={difference:.8g}",
...
def paired_absrel(sequence_rows, datasets, samples, seed):
    ...
    for dataset in datasets:
        ...
        names_stac = {name for name, method in index if method == "StreamVGGT-STAC"}
        if names_k4 != names_stac:
            raise RuntimeError(
                f"coverage mismatch for {dataset}: K4={sorted(names_k4)}, STAC={sorted(names_stac)}"
            )
        names = sorted(names_k4)
        k4 = np.asarray([index[(name, "K4")] for name in names])
        stac = np.asarray([index[(name, "StreamVGGT-STAC")] for name in names])
        advantage = stac - k4
        ...
        outputs.append(
            {
                ...
                "mean_k4": float(k4.mean()), "mean_stac": float(stac.mean()),
                ...
                "stac_wins": int(np.sum(advantage < -1e-12)),
...
def comparisons(aggregate_rows, datasets):
    ...
    for dataset in datasets:
        k4 = index[(dataset, "K4")]
        stac = index[(dataset, "StreamVGGT-STAC")]
        for metric, direction in directions.items():
            a, b = float(k4[metric]), float(stac[metric])
            winner = "K4" if (a < b if direction == "lower" else a > b) else "StreamVGGT-STAC"
            if abs(a - b) <= 1e-12:
                winner = "tie"
            rows.append(
                {
                    "dataset": dataset, "metric": metric, "direction": direction,
                    "k4": a, "stac": b, "k4_minus_stac": a - b,
...
def main():
    ...
    stac_aggregate, stac_sequences = load_stac(root, output_root, datasets)
    aggregates = k4_aggregate + stac_aggregate
    sequences = k4_sequences + stac_sequences
    if len(aggregates) != len(datasets) * 2:
        raise RuntimeError(f"incomplete aggregate matrix: {len(aggregates)} rows")
    for row in stac_aggregate:
        if int(row["num_failed"]):
            raise RuntimeError(f"STAC has failed sequences: {row}")
    ...
    gate_rows.append(
        {
            ...
            "observed": "; ".join(f"{row['dataset']}={row['num_successful']}" for row in stac_aggregate),
            "threshold": "K4 and STAC sequence/frame signatures identical",
            "action": "continue",
        }
    )
    ...
    write_csv(
        ...
        ("dataset", "metric", "direction", "k4", "stac", "k4_minus_stac", "k4_relative_change_pct", "winner"),
        comparison_rows,
    )
    write_csv(
        root / "stage5e_paired_statistics.csv",
        ("dataset", "metric", "n_pairs", "mean_k4", "mean_stac", "mean_advantage_k4", "ci95_low", "ci95_high", "k4_wins", "ties", "stac_wins", "bootstrap_samples", "bootstrap_seed"),
        paired_rows,
    )
    write_csv(
